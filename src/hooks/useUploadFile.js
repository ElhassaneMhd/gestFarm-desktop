...
export function useUploadFile({ options = defaultOptions, onChange, onError }) {
  const accept = (options.accept || defaultOptions.accept).map((type) =>
    type.replace(".", "")
  );
  const { openFilePicker } = useFilePicker({
    ...
    validators: [
      ...
      new FileTypeValidator([
        ...accept,
        ...accept.map((type) => type.toUpperCase()),
      ]),
  ...
  const upload = async (file) => {
    ...
    formData.append("upload_preset", "gestfarm");
    formData.append("cloud_name", "dwlbpv5xp");
    formData.append("file", file);
    const res = await fetch(
      "https://api.cloudinary.com/v1_1/dwlbpv5xp/image/upload",
      {
        method: "POST",
        body: formData,
      }
  ...
  return { openFilePicker, options, upload };
}
...
function getErrorMessage(
  ...
  switch (error) {
    ...
    case "FileAmountLimitError":
      return `Only ${
        maxFiles === 1 ? "one" : maxFiles
      } ${type.toLowerCase()} is allowed`;
    default:
