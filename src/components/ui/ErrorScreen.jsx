...
import { Files, Layers, Layers2 } from "lucide-react";
...
export function ErrorScreen({ error }) {
  ...
  return (
    <>
      <div className="flex h-screen w-full max-w-screen-xl items-center justify-center bg-background-primary px-4 md:px-8">
        <div className="max-w-xl space-y-5 text-center">
          ...
          <p className="text-text-secondary">
            Oops! Something unexpected happened. We&apos;re working on getting
            it fixed.{" "}
            <span className="font-bold text-text-primary">Thanks</span> for your
            patience!
          </p>
          ...
          {control.isDeveloper && (
            <div className="relative mt-4 max-h-[200px] overflow-auto rounded-lg border border-border bg-background-secondary p-3 text-sm text-text-secondary">
              <div className="absolute right-2 top-2 flex">
                <ToolTip
                  content={
                    control.showDetails ? "Hide Details" : "Show Details"
                  }
                >
                  <Button
                    shape="icon"
      ...
      <Toaster
        position={
          JSON.parse(localStorage.getItem("local_settings"))?.toastPosition ||
          "bottom-right"
        }
